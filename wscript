...
VERSION = "0.4.1"
APPNAME = "ndn-chunks"
PACKAGE_BUGREPORT = "https://redmine.named-data.net/projects/ndn-chunks"
PACKAGE_URL = "http://named-data.net/doc/ndn-chunks/"
GIT_TAG_PREFIX = "ndn-chunks-"
...
''')
    ...
    if not conf.env.enable_static:
        ...
        # system has a different version of the ndn-chunks library installed.
    ...
    conf.write_config_header('src/ndn-chunks-config.hpp', define_prefix='NDN_CHUNKS_')
...
def build(bld):
    ...
    pkgconfig_includes = []
    pkgconfig_cxxflags = []
    for lib in Utils.to_list(libndn_chunks['use']):
    ...
    pkconfig = bld(features="subst",
         source="libndn-chunks.pc.in",
         target="libndn-chunks.pc",
    ...
    bld.install_files("%s/ndn-chunks" % bld.env['INCLUDEDIR'],
                      bld.path.find_resource('src/ndn-chunks-config.hpp'))
